...
public class WatermarkHelper extends PdfPageEventHelper {
     ...
     */
    public WatermarkHelper(String text, int fontSize, float alpha, int angle, boolean flip, int x,
    ...
    public void onEndPage(PdfWriter writer, Document document) {
        ...
        if (text != null) {
            ...
            ColumnText.showTextAligned(writer.getDirectContentUnder(), Element.ALIGN_CENTER, phrase, x, y,
                    (flip ? (writer.getPageNumber() % 2 == 1 ? angle : -angle) : angle));
        } else {
            if (x == -1 && y == -1) {
