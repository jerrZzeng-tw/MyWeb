    ...
    static {
        try {
            ...
            if (fontInitial.booleanValue()) {
                ...
                baseFontMap.put(fontPath + fontName,
                        BaseFont.createFont(fontPath + fontName, BaseFont.IDENTITY_H, BaseFont.EMBEDDED));

                if (StringUtils.isNotBlank(
                        cns11643FontPath) && cns11643SungFontName != null && cns11643SungFontName.length > 0) {
                    cnsSungBaseFontList = new ArrayList<BaseFont>();
                    for (String font : cns11643SungFontName) {
                        cnsSungBaseFontList.add(
                                BaseFont.createFont(cns11643FontPath + font, BaseFont.IDENTITY_H, BaseFont.EMBEDDED));
                    }
                }

                if (StringUtils.isNotBlank(
                        cns11643FontPath) && cns11643KaiFontName != null && cns11643KaiFontName.length > 0) {
