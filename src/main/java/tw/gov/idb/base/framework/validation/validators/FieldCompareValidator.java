...
import tw.gov.idb.base.framework.validation.FieldCompare;
...
public class FieldCompareValidator implements ConstraintValidator<FieldCompare, Object> {
    ...
    public boolean isValid(Object value, ConstraintValidatorContext context) {
        ...
        if (fromClass.equals(thenClass) && ClassUtils.isAssignableValue(Comparable.class,
                fromObject) && ClassUtils.isAssignableValue(Comparable.class, thenObject)) {
            ...
            if (!nullable) {
                if (fromValue == null && thenValue != null) {
                    ...
                    context.buildConstraintViolationWithTemplate("{validation.fieldCompare.requiredFrom.message}")
                            .addConstraintViolation();
                    return false;
                } else if (fromValue != null) {
                    context.disableDefaultConstraintViolation();
                    context.buildConstraintViolationWithTemplate("{validation.fieldCompare.requiredThen.message}")
                            .addConstraintViolation();
                    return false;
                }
            }
