...
public class DateUtility {
    ...
    public static int lastDay(String sDate, boolean bBeforeROC) {
        ...
        try {
            if (sDate.length() == 7) {
                if (bBeforeROC)
                    sDate = (1912 - Integer.parseInt(sDate.substring(0, 3))) + sDate.substring(3, 7);
                else
                    sDate = (1911 + Integer.parseInt(sDate.substring(0, 3))) + sDate.substring(3, 7);
            }
    ...
    public static String changeDateType(String sDate, boolean bBeforeROC) {
        ...
        if (sDate.length() == 7) {
            if (bBeforeROC)
                sReturnDate = StringUtils.leftPad(String.valueOf(1912 - Integer.parseInt(sDate.substring(0, 3))), 4,
                        "0") + sReturnDate;
            else
                sReturnDate = StringUtils.leftPad(String.valueOf(Integer.parseInt(sDate.substring(0, 3)) + 1911), 4,
                        "0") + sReturnDate;
        } else if (sDate.length() == 8) {
            if (Integer.parseInt(sDate.substring(0, 4)) < 1912) // 民前
                sReturnDate = StringUtils.leftPad(String.valueOf(1912 - Integer.parseInt(sDate.substring(0, 4))), 3,
                        "0") + sReturnDate;
            else
                sReturnDate = StringUtils.leftPad(String.valueOf(Integer.parseInt(sDate.substring(0, 4)) - 1911), 3,
                        "0") + sReturnDate;
        }
    ...
    public static String formatChineseYearMonthString(String sDate, boolean bChinese) {
        if (StringUtils.defaultString(sDate).trim().length() != 5 && StringUtils.defaultString(sDate)
                .trim()
                .length() != 7) {
            return StringUtils.defaultString(sDate);
        }
    ...
    public static String formatWestYearMonthString(String sDate, boolean bChinese) {
        if (StringUtils.defaultString(sDate).trim().length() != 6 && StringUtils.defaultString(sDate)
                .trim()
                .length() != 8) {
            return StringUtils.defaultString(sDate);
        }
    ...
    public static String formatChineseDateTimeString(String sDateTime, boolean bChinese) {
        ...
        String sTime = "";
        // 西元先轉成 民前 或 民國
        if (StringUtils.defaultString(sDateTime).length() == 8 || StringUtils.defaultString(sDateTime)
                .length() == 12 || StringUtils.defaultString(sDateTime).length() == 14) {
        ...
        } else if (StringUtils.defaultString(sDateTime).length() == 7 || StringUtils.defaultString(sDateTime)
                .length() == 11 || StringUtils.defaultString(sDateTime).length() == 13) {
    ...
    public static String formatWestDateTimeString(String sDateTime, boolean bBeforeROC) {
        ...
        String sTime = "";
        // 七碼先轉成西元
        if (StringUtils.defaultString(sDateTime).length() == 7 || StringUtils.defaultString(sDateTime)
                .length() == 11 || StringUtils.defaultString(sDateTime).length() == 13) {
            sDate = changeDateType(sDateTime.substring(0, 7), bBeforeROC);
            sTime = sDateTime.substring(7);
        } else if (StringUtils.defaultString(sDateTime).length() == 8 || StringUtils.defaultString(sDateTime)
                .length() == 12 || StringUtils.defaultString(sDateTime).length() == 14) {
    ...
    public static String parseLocalDateToChineseDateTime(LocalDate localDate) {
        return parseDateToChineseDateTime(
    ...
    public static String getChineseTimeString(String time, boolean bChinese) {
        if (StringUtils.length(time) != 4 && StringUtils.length(time) != 6)
        ...
        if (hours == 0) {
            ...
            if (bChinese)
                return "午夜 " + DateUtility.formatTimeString(newTime, false);
            else
                return DateUtility.formatTimeString(newTime, false) + " midnight";
        } else if (hours > 0 && hours < 12) {
            String newTime = StringUtils.leftPad(String.valueOf(hours), 2, "0") + StringUtils.substring(time, 2);
            if (bChinese)
                return "上午 " + DateUtility.formatTimeString(newTime, false);
            else
                return DateUtility.formatTimeString(newTime, false) + " AM";
        } else if (hours == 12) {
            String newTime = StringUtils.leftPad(String.valueOf(hours), 2, "0") + StringUtils.substring(time, 2);
            if (bChinese)
                return "中午 " + DateUtility.formatTimeString(newTime, false);
            else
                return DateUtility.formatTimeString(newTime, false) + " noon";
        } else {
            String newTime = StringUtils.leftPad(String.valueOf(hours - 12), 2, "0") + StringUtils.substring(time, 2);
            if (bChinese)
                return "下午 " + DateUtility.formatTimeString(newTime, false);
            else
                return DateUtility.formatTimeString(newTime, false) + " PM";
        }
    }
    ...
    public static String getDateTimeString(Timestamp timestamp, boolean bChineseYear, boolean bChineseTime) {
        ...
        String dateStrFormat =
                bChineseYear ? formatChineseDateTimeString(dateStr, false) : formatWestDateString(dateStr, false);
    ...
    /**
     * case set jsp輸入時用 民國年時判斷是否可以轉成西元年<p>
     * 無法轉則傳回原先的字串<p>
     * 搭配@WestDate 和  @RequiredInteger 用
     *
     * @param date
     * @return
     */
    public static String changeChineseDateForCaseSet(String date) {
        date = StringUtils.trim(date);
        if (date != null && RegexUtility.isMatches(date, PATTERN) && date.length() == 7)
            return changeDateTypeToWestDate(date);
        else
            return date;
    }

    /**
     * case get jsp顯示時用 西元年時判斷是否可以轉成民國年<p>
     * 無法轉則傳回原先的字串<p>
     *
     * @param date
     * @return
     */
    public static String changeWestDateForCaseGet(String date) {
        if (date != null && RegexUtility.isMatches(date, PATTERN) && date.length() == 8)
            return changeDateTypeToChineseDate(date);
        else
            return date;
    }
     ...
     * @return
     */
    public static String changeWestYearMonthTypeForCaseGet(String yearMonth) {
        if (StringUtils.defaultString(yearMonth).equals("") || yearMonth.length() != 6 || (!RegexUtility.isMatches(
                yearMonth, PATTERN)))
            return yearMonth;
        else {
            String year = yearMonth.substring(0, 4);
     ...
     * @return
     */
    public static String changeChineseYearMonthTypeForCaseSet(String yearMonth) {
        if (StringUtils.defaultString(yearMonth).equals("") || yearMonth.length() != 5 || (!RegexUtility.isMatches(
                yearMonth, PATTERN)))
            return yearMonth;
        else {
            String year = yearMonth.substring(0, 3);
     ...
     * @return
     */
    public static String changeYearTypeForCase(String year) {
        if (StringUtils.length(year) == 4 && RegexUtility.isMatches(year, PATTERN)) {
            return StringUtils.substring(changeDateType(year + "0101"), 0, 3);
        } else if (StringUtils.length(year) == 3 && RegexUtility.isMatches(year, PATTERN)) {
            return StringUtils.substring(changeDateType(year + "0101"), 0, 4);
        } else {
            return year;
        }
    ...
    /**
     * 年齡計算 以當天來計算 <br>
     * 出身19961001 申請19971001 1歲 <br>
     * 出身19961001 申請19971010 1歲 <br>
     * 出身19961001 申請19980901 1歲 <br>
     *
     * @param appdate   傳入西元日期
     * @param evtBrdate 傳入西元日期
     * @return
     */
    public static String calEvtAge(String appdate, String evtBrdate) {
        if (StringUtils.isAnyBlank(appdate, evtBrdate) || StringUtils.length(appdate) != 8 || StringUtils.length(
                appdate) != 8) {
            return "0";
        }

        LocalDate appLocalDate = LocalDate.parse(appdate, DateTimeFormatter.BASIC_ISO_DATE);
        LocalDate evtLocalDate = LocalDate.parse(evtBrdate, DateTimeFormatter.BASIC_ISO_DATE);

        return String.valueOf(Period.between(evtLocalDate, appLocalDate).getYears());
    }
