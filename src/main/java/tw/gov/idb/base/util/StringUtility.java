...
public class StringUtility {
    ...
    public static String chtTrimToEmpty(String str) {
        ...
        for (int i = 0; i < str.length(); i++) {
            if (!StringUtils.equals(StringUtils.substring(str, i, i + 1), " ") && !StringUtils.equals(
                    StringUtils.substring(str, i, i + 1), "　")) {
                beginIndex = i;
                break;
            }
        }
        ...
        for (int i = str.length(); i >= 0; i--) {
            if (!StringUtils.equals(StringUtils.substring(str, i - 1, i), " ") && !StringUtils.equals(
                    StringUtils.substring(str, i - 1, i), "　")) {
                endIndex = i;
                break;
            }
        }
    ...
    private static String integer2TWD(String integer) {
        ...
        for (i = integer.length() - 1, j = 0; i >= 0; i--, j++) {
            ...
            if (n == '0') {
                ...
                // 插入[萬]或者[億]
                if (j % 4 == 0) {
                    if (i > 0 && integer.charAt(i - 1) != '0' || i > 1 && integer.charAt(
                            i - 2) != '0' || i > 2 && integer.charAt(i - 3) != '0') {
    ...
    public static String safeLog(String msg) {
        return removeControlCharacter(StringUtils.defaultString(msg));
    }
        ...
        for (int i = 0; i < ca.length; i++) {
            ...
            switch (ca[i]) {
                ...
                case 32:      //半型空白轉成全型空白
                    ...
                    continue outer;
                default:
                    if (Character.isLetterOrDigit(ca[i])) { //數字或字母
     ...
     * @return
     */
    public static String beforeAppend(String append, String str) {
        return StringUtils.isBlank(str) ? StringUtils.EMPTY : append + str;
    }
     ...
     * @return
     */
    public static String afterAppend(String append, String str) {
        return StringUtils.isBlank(str) ? StringUtils.EMPTY : str + append;
    }
     ...
     * @return
     */
    public static String afterBeforeAppend(String str, String before, String after) {
        return StringUtils.isBlank(str) ? StringUtils.EMPTY : before + str + after;
    }

    /**
     * 用分隔符號接合字串陣列
     *
     * @param strArray  要串接的字串陣列 選傳 (沒傳就回傳null)
     * @param delimiter 分隔符號(ex. '-' ',' ...) 選填
     * @param blankShow 空白是否要show
     */
    public static String joining(String[] strArray, String delimiter, boolean blankShow) {
        if (strArray == null) {
            return null;
        }

        return joining(Stream.of(strArray)
                .filter(s -> blankShow || StringUtils.isNotBlank(s))
                .toArray(size -> new String[size]), delimiter);
    }
     ...
     * @param delimiter 分隔符號(ex. '-' ',' ...) 選填
     */
    public static String joining(String[] strArray, String delimiter) {
        if (strArray == null) {
            return null;
        }
        return Arrays.stream(strArray)
                .map(s -> StringUtils.defaultString(s, HALF_SPACE))
                .collect(Collectors.joining(StringUtils.defaultString(delimiter)));
    }
     ...
     * @param strArray  要串接的字串陣列 選傳 (沒傳就回傳null)
     * @param delimiter 分隔符號(ex. '-' ',' ...) 選填
     * @param prefix    前綴(ex. '(' '[' ...) 選填
     * @param suffix    後綴(ex. ')' ']' ...) 選填
     */
    public static String joining(String[] strArray, String delimiter, String prefix, String suffix) {
        if (strArray == null) {
            return null;
        }
        return Arrays.stream(strArray)
                .map(s -> StringUtils.defaultString(s, HALF_SPACE))
                .collect(Collectors.joining(StringUtils.defaultString(delimiter), StringUtils.defaultString(prefix),
                        StringUtils.defaultString(suffix)));
    }

    /**
     * 處裡難字: 將超過BIG5能顯示的難字換成全型問號
     *
     * @param inputStr
     * @return
     * @throws Exception
     */
    public static String trimToBig5Charset(String inputStr) throws Exception {
